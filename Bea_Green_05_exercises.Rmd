...
summarize(tot_trips = sum(total_num_trips)) %>%
  arrange(desc(tot_trips)) %>% 
  slice(1:20)
```

%>%
fct_infreq(departure_station) %>% 
  ggplot(aes(x = tot_trips, y = arrival_station)) +
  geom_point() +
  facet_grid(vars(departure_station))
  
ggplot(aes(x = avg_delay_all_departing, y = avg_delay_all_arriving)) +
  geom_point() +
