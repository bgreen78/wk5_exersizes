...
small_trains %>% 
  ...
  labs(title = "How Many Trains Arrived in the Highest Trafficked\n Stations in France from 2015-2018?",
       x = "",
  ...
  theme(plot.background = element_rect(fill = "darkblue"),
        ...
        axis.title = element_text(color = "ivory"),
        plot.title = element_text(color = "ivory", size = 14),
        plot.title.position = "plot",
        plot.subtitle = element_text(color = "ivory", size = 10))
```

```{r, eval=FALSE, echo=FALSE}
anim_save("small_trains_graph2.gif")
```

```{r, echo=FALSE}
knitr::include_graphics("small_trains_graph2.gif")
```
...
```{r}
panama_all <- bind_rows(panama_run, panama_bike, panama_swim)
```
...
ggmap(panama_map) +
  ...
  transition_reveal(time)
```
LINE JUMPING--WRONG TIMES SHOWING UP. SOMETHING IS WRONG. RECOLOR ENTRY BY EVENT.
...
covid_log_graph <- covid19 %>% 
  group_by(state) %>%
  mutate(seven_day_lag = lag(cases, n=7), week_cases = cases-seven_day_lag) %>% 
  replace_na(list(seven_day_lag = 0, week_cases = 0)) %>% 
  filter(cases>20) %>% 
 ...
 **Comment**: In this graph I see WHAT???? 
